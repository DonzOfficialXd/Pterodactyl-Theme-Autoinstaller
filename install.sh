...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] ===============================================${NC}"
  echo -e "${BLUE}[+]               LICENSY DONZY XD CODE             ${NC}"
  echo -e "${BLUE}[+] ===============================================${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "donzyxd" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di DonzyXd${NC}"
    echo -e "${YELLOW}TELEGRAM : @DonzyNeedBuyer${NC}"
    echo -e "${YELLOW}WHATSAPP : +628881012163184${NC}"
    echo -e "${YELLOW}© DonzyXd${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer DonzyTzy   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : DonzyNeedBuyer      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : DonzyTzy  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by DonzyXd  ${NC}"
